...
function getSplit(daysPerWeek) {
  const FULL_A = { focus: "Ganzkörper A", slots: ["push", "pull", "legs", "core"] };
  const FULL_B = { focus: "Ganzkörper B", slots: ["dips", "pushVertical", "hamstrings", "core"] };
  const FULL_C = { focus: "Ganzkörper C", slots: ["push", "pull", "legs", "hamstrings"] };
  const PUSH   = { focus: "Push (Drücken)", slots: ["push", "pushVertical", "dips", "core"] };
  const PULL   = { focus: "Pull (Ziehen)", slots: ["pull", "hamstrings", "core"] };
  const LEGS   = { focus: "Legs (Beine)", slots: ["legs", "hamstrings", "core"] };
  const UPPER  = { focus: "Oberkörper", slots: ["push", "pull", "dips", "pushVertical"] };
  const LOWER  = { focus: "Unterkörper & Core", slots: ["legs", "hamstrings", "core"] };
  switch (Number(daysPerWeek)) {
    ...
    default: return [FULL_A, FULL_B, FULL_C];
  }
}
...
function adjustTraining(training, key, score, effort) {
  ...
  if (!t) return { changed: 0, leveledUp: false, leveledDown: false };
  ...
  if (score < 0.5 && (score <= 0.25 || effort === "hard")) {
    // Klar überfordert -> sofort leichtere Variante
    if (t.level > 0) { t.level--; t.reps = Math.round((def().min + def().max) / 2); }
    else { t.reps = Math.max(def().min, t.reps - 2); }
  } else if (score >= 0.999) {
    ...
    if (effort === "hard") {
      ...
      t.reps += 1;
      // Sehr leicht und schon am oberen Ende -> direkt nächste Variante
      if (effort === "easy" && t.reps >= def().max && t.level < lastLevel) { t.level++; t.reps = def().min; }
    } else if (t.level < lastLevel) {
      t.level++; t.reps = def().min;                 // Spanne ausgereizt -> schwerere Übung
    } else {
      t.reps = Math.min(t.reps + 1, def().max + (def().type === "hold" ? 15 : 3));
    }
  } else if (score >= 0.5) {
    // Teilweise geschafft -> Ziel wiederholen; bei RPE 9-10 leicht zurück
    if (effort === "hard" && t.reps > def().min) t.reps -= 1;
  } else {
    // Unter der Hälfte (nicht als „schwer" markiert) -> 1 Wdh. zurück
    if (t.reps > def().min) t.reps -= 1;
    else if (t.level > 0) { t.level--; t.reps = def().max; }
  }

  // Sicher in gültiger Spanne halten
  ...
  return { changed: 0, leveledUp: t.level > startLevel, leveledDown: t.level < startLevel };
}
